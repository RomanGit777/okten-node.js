...
// console.log(process.cwd()); //current working branch : c:/users/rsave/webStorm/okten-node.js
...
// console.log(path.normalize('C:\\\\Users\\\\rsave\\\\WebstormProjects\\\\okten-node.js\''));
...
// const start = async ()=> {
    ...
    // const filePath = path.join('storage', 'aad.txt');
    ...
    // await fs.rename(filePath, path.join(process.cwd(),'storage', 'asd','myFile3.txt'));
    // await fs.rename(filePath, path.join(path.dirname(filePath), 'aad.txt'))
...
//         await rl.close()
//     }
//     const readStream = fs.createReadStream('storage/aad.txt');
//     const writeStream = fs.createWriteStream('res2.txt');
